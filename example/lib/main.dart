...
void main() {
  runApp(MaterialApp(home: MaterialBarcodeScanner()));
}
...
class _MaterialBarcodeScannerState extends State<MaterialBarcodeScanner>
  ...
  final BarcodeDetector _barcodeDetector = FlutterQr.instance.barcodeDetector();
  ...
  void _handleResult({
    ...
    for (Barcode barcode in barcodes) {
      if (barcode.url != null) {
        print('GOT URL BARCODE: ${barcode.url}');
      }
      final Rect intersection = validRect.intersect(barcode.boundingBox);

      final bool doesContain = intersection == barcode.boundingBox;
      print('VALID RECT: $validRect');
      print('INTERSECTION: $intersection');
      print('BAR CODE BOUNDING BOX: ${barcode.boundingBox}');
      print('DOES CONTAIN: $doesContain');
      if (doesContain) {
