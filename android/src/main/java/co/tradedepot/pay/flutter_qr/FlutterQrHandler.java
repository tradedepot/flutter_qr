...
class FlutterQrHandler implements MethodChannel.MethodCallHandler {
    private final SparseArray<MyBarcodeDetector> detectors = new SparseArray<>();
    private final Context applicationContext;

    FlutterQrHandler(Context applicationContext) {
    ...
    private void handleDetection(MethodCall call, MethodChannel.Result result) {
        ...
        // Log.d("FlutterQrHandler", "DETECT: "+options);
        InputImage image;
      ...
      MyBarcodeDetector detector = getDetector(call);
        if (detector == null) {
            detector = new MyBarcodeDetector(options);
    ...
    private void closeDetector(final MethodCall call, final MethodChannel.Result result) {
        final MyBarcodeDetector detector = getDetector(call);
        Log.d("FlutterQrHandler", "CLOSE DETECTOR");
        if (detector == null) {
            final Integer handle = call.argument("handle");
    ...
    private InputImage dataToVisionImage(Map<String, Object> imageData) throws IOException {
        ...
        switch (imageType) {
            ...
            case "bytes":
                ...
                return InputImage.fromByteArray(bytes,
                        (int) (double) metadataData.get("width"),
                        (int) (double) metadataData.get("height"),
                        (int) metadataData.get("rotation"),
    ...
    private void addDetector(final int handle, final MyBarcodeDetector detector) {
    ...
    private MyBarcodeDetector getDetector(final MethodCall call) {
        final Integer handle = call.argument("handle");
