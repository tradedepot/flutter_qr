...
///
/// You can get an instance by calling [FlutterQr.instance] and then get
/// a detector from the instance:
///
/// ```dart
/// TextRecognizer textRecognizer = FlutterQr.instance.textRecognizer();
/// ```
class FlutterQr {
  FlutterQr._();
  ...
  static const MethodChannel channel =
      MethodChannel('co.tradedepot.pay.flutter_qr');

  @visibleForTesting
  static int nextHandle = 0;

  /// Singleton of [FlutterQr].
  ///
  ...
  /// TextRecognizer textRecognizer = FlutterQr.instance.textRecognizer();
  /// ```
  static final FlutterQr instance = FlutterQr._();
...
class FlutterQrImage {
  FlutterQrImage._({
    @required _ImageType type,
    FlutterQrImageMetadata metadata,
    File imageFile,
    Uint8List bytes,
  ...
  /// Construct a [FlutterQrImage] from a file.
  factory FlutterQrImage.fromFile(File imageFile) {
    assert(imageFile != null);
    return FlutterQrImage._(
      type: _ImageType.file,
      imageFile: imageFile,
    );
  }

  /// Construct a [FlutterQrImage] from a file path.
  factory FlutterQrImage.fromFilePath(String imagePath) {
  ...
  /// Construct a [FlutterQrImage] from a list of bytes.
  ///
  ...
  factory FlutterQrImage.fromBytes(
    Uint8List bytes,
    FlutterQrImageMetadata metadata,
  ) {
    assert(bytes != null);
  ...
  final FlutterQrImageMetadata _metadata;
...
/// if `null`.
class FlutterQrImagePlaneMetadata {
  FlutterQrImagePlaneMetadata({
...
/// Image metadata used by [FlutterQr] detectors.
///
...
/// `null`.
class FlutterQrImageMetadata {
  FlutterQrImageMetadata({
    @required this.size,
  ...
  ///
  /// Not used on Android.
  final List<FlutterQrImagePlaneMetadata> planeData;
  ...
  Map<String, dynamic> _serialize() => <String, dynamic>{
        ...
        'rawFormat': rawFormat,
        'planeData': planeData
            .map((FlutterQrImagePlaneMetadata plane) => plane._serialize())
            .toList(),
      };
}
