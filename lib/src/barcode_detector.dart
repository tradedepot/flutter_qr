...
class BarcodeFormat {
  ...
  static const BarcodeFormat all = BarcodeFormat._(0);

  /// Barcode format unknown to the current SDK.
  static const BarcodeFormat unknown = BarcodeFormat._(0xFFFF);
...
/// `barcodeDetector([BarcodeDetectorOptions options])` in [FlutterQr]:
///
/// ```dart
/// final FlutterQrImage image =
///     FlutterQrImage.fromFilePath('path/to/file');
///
/// final BarcodeDetector barcodeDetector =
///     FlutterQr.instance.barcodeDetector();
///
...
class BarcodeDetector {
  ...
  Future<List<Barcode>> detectInImage(FlutterQrImage visionImage) async {
    assert(!_isClosed);

    _hasBeenOpened = true;
    final List<dynamic> reply =
        await FlutterQr.channel.invokeListMethod<dynamic>(
      'detectInImage',
      <String, dynamic>{
        'handle': _handle,
  ...
  Future<void> close() {
    ...
    _isClosed = true;
    return FlutterQr.channel.invokeMethod<void>(
      'close',
...
class BarcodeDetectorOptions {
  const BarcodeDetectorOptions({this.barcodeFormats = BarcodeFormat.qrCode});
